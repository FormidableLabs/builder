...
Task.prototype.execute = function (callback) {
  var env = this.env.env; // Raw environment object.
  ...
  if (this.action === "run") {
    ...
    return runner.run(task, { env: env }, callback);
  }
  ...
  if (this.action === "concurrent") {
    ...
    return runner.concurrent(tasks, { env: env }, callback);
  }

  if (this.action === "install") {
    var paths = this.config.archetypeNodePaths;
    log.info(this.action, "Install dev dependencies for:" + paths.map(function (p) {
      return "\n * " + chalk.gray(p);
    }).join(""));

    return runner.install(paths, { env: env }, callback);
  }
