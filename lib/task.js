...
Task.prototype.run = function (callback) {
  ...
  async.series([
    task.pre && run.bind(null, task.pre.cmd, { env: env }, preOpts),
    run.bind(null, task.main.cmd, { env: env }, mainOpts),
    task.post && run.bind(null, task.post.cmd, { env: env }, postOpts)
...
Task.prototype.concurrent = function (callback) {
  ...
  var mainOpts = this.getMainOpts(tasks[0].main || {}, flags);
...
Task.prototype.envs = function (callback) {
  ...
  // Run.
  this._runner.envs(task.main.cmd, { env: env }, mainOpts, callback);
};
