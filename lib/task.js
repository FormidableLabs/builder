...
Task.prototype.concurrent = function (callback) {
  ...
  var skipIfKilled = function (fn) {
    return function (cb) {
      return self._tracker.killed || setupKilled ? cb() : fn(cb);
    };
  };
  ...
  async.mapLimit(paramsList, queue || Infinity, function (params, concCb) {
    ...
    async.series([
      ...
      function (cb) {
        var proc = startSetup(params.main.opts, shOpts, callback);
        if (proc) {
          proc.on("close", function () { setupKilled = true; });
        }
        cb();
      },
    ...
    ].filter(Boolean).map(skipIfKilled), self._errOrBail(params.main.opts, concCb));
  }, callback);
};
