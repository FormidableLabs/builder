...
Task.prototype.concurrent = function (callback) {
  ...
  var bail = firstParams.main.opts.bail;
  ...
  async.mapLimit(paramsList, queue || Infinity, function (params, concCb) {
    ...
    async.series([
      ...
      function (cb) {
        ...
        return opts.tries === 1 ?
          self.trackAndRun(params.main.cmd, shOpts, opts, cb) :
          runner.retry(params.main.cmd, shOpts, opts, function (err) {
            if (err) {
              self._errors.push(err);
            }
            cb(bail ? err : null); // Decide if failure ends the run.
          });
      },

      // Execute `post<task>`
      params.post && self.trackAndRun.bind(self, params.post.cmd, shOpts, params.post.opts)
    ].filter(Boolean), concCb);
  }, callback);
};
  ...
  async.series([
    ...
    function (cb) {
      ...
      async.mapLimit(taskEnvs, queue || Infinity, function (taskEnv, envCb) {
        ...
        runner.retry(cmd, taskShOpts, taskOpts, function (err) {
          if (err) {
            self._errors.push(err);
          }
          envCb(bail ? err : null); // Decide if failure ends the run.
        });
      }, cb);
    },
