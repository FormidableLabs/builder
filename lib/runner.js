...
var addSetup = function (setup, shOpts) {
  ...
  var argv = [null, "builder", "run", setup];
  ...
  // Run it!
  var proc = run(main.cmd, { env: env.env }, { argv: argv }, done);
  proc.on("exit", done);
...
var createFinish = function (shOpts, opts, tracker, callback) {
  ...
  var setup = tracker.add(runner.addSetup(opts.setup, shOpts));
  if (setup) {
...
runner = module.exports = {
  ...
  // Route through object for test stubbing.
  addSetup: addSetup,
  ...
  run: function (cmd, shOpts, opts, callback) {
    ...
    if (tries === 1) {
      tracker.add(run(cmd, shOpts, opts, finish));
      return;
    }
   ...
   *
   * **Note**: `pre/post` lifecycle tasks are run as a group with the
   * underlying main task, meaning they occupy the queue for that entire time.
   *
