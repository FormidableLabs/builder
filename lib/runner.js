...
/**
 * Merge custom commands (`--`) into chosen script command.
 *
 ...
 * @returns {String}    Updated command
 */
var expandArchetype = function (cmd, opts) {
  // Get token to expand, otherwise.
  opts = opts || {};
  var expand = opts.expandArchetype;
  ...
  var archToken = opts._archetypeName;
  if (!archToken) {
  ...
  console.log("TODO HERE PARSED", expand, archToken);

  // TODO HACK:
  archToken = "node_modules";
  var archFull = "HI";

  // // Use literal string matching to avoid RegExp escaping.
  // var temp = cmd.split(archToken).map(function (part, i) {
  //   // Check previous part
  //   if (i === 0) {
  //     return archFull + part;
  //   }

  //   // Need more checks for space in front of previous part. TODO
  //   console.log("TODO HERE", i, part);

  //   return archToken + part;
  // }).join("");

  // console.log("TODO HERE TEMP BROKEN", temp);
...
var run = function (cmd, shOpts, opts, callback) {
  // Mutation steps for command.
  // (Separated for easier ordering / testing / identification).
  cmd = cmdWithCustom(cmd, opts);
  cmd = expandArchetype(cmd, opts);
