...
var replaceToken = function (str, token, sub) {
  var tokenRe = new RegExp("^" + _.escapeRegExp(token), "g");
  return str.replace(tokenRe, function (match/* offset, origStr*/) {
    // Sanity check.
    if (match !== token) {
...
var expandArchetype = function (cmd, opts) {
  ...
  var archFull = "TODO_FULL_ARCH_PATH";

  return replaceToken(cmd, archToken, archFull);
};
