...
var run = function (cmd, shOpts, opts, callback) {
  ...
  var proc = exec(cmd, shOpts, function (err, stdout, stderr) {
    ...
    log[level]("proc:end:" + code, cmdMsg);
    callback(err);
  });
  ...
  async.whilst(
    ...
    function (cb) {
      var proc = run(cmd, shOpts, opts, function (err) {
        ...
        // Check tries.
        if (error && tries > 0) {
          log.warn("proc:retry", chalk.red(tries) + " tries left, " + cmdMsg);
        }
    ...
    function (err) {
      ...
      if (error) {
        ...
        log.error("proc:error", "Code: " + code + ", " + cmdMsg);
      }
...
module.exports = {
  ...
  envs: function (cmd, shOpts, opts, callback) {
    ...
    async.mapLimit(taskEnvs, queue || Infinity, function (taskEnv, cb) {
      ...
      finish(errors.length ? errors[0] : null);
    });
  }
};
